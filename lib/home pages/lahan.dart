...
class _LahanPageState extends State<LahanPage> {
  ...
  Future<void> _deleteLand(String landId) async {
    ...
    final response = await http.delete(
      Uri.parse('http://bfarm.ahmadyaz.my.id/api/user-land/delete/$landId'),
      headers: {
        'Authorization': 'Bearer $token',
      },
    );
    ...
    if (response.statusCode == 200) {
      // Hapus lahan dari daftar dan perbarui UI
      setState(() {
        _landData.removeWhere((land) => land['id'] == landId);
      });
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Lahan berhasil dihapus')),
      );
    } else {
      // Tangani kesalahan API
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Gagal menghapus lahan')),
      );
    }
  }

  void _confirmDelete(String landId, String landDescription) {
    showDialog(
      context: context,
